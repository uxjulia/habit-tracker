...
export async function login(username: string, password: string) {
  const res = await apiClient.post<{ data: { token: string; username: string }; error: null }>("/auth/login", {
    username,
    password,
  });
...
export async function setup(username: string, password: string) {
  const res = await apiClient.post<{ data: { token: string; username: string }; error: null }>("/auth/setup", {
    username,
    password,
  });
  return res.data.data;
